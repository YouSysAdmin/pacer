...
export function isPosixUser(s) {
  if (!s) return true;
  return POSIX_USER_RE.test(s);
}

export function isPoolName(s) {
  if (!s) return false;
...
export function noSlashOrSpace(s) {
  return !/[/ \t]/.test(s || "");
}
...
export function isRepoFullName(s) {
  if (!s) return false;
  const i = s.indexOf("/");
...
// on pool ExtraLabels.
export function notSelfHosted(s) {
  return sanitizeLabel(s) !== "self-hosted";
}

// --- err.fields helpers -----------------------------------------------

// fieldErrorsFrom turns an Error thrown by api.js (with the optional
// .fields array from the validator's structured envelope) into a flat
// {fieldName: message} map. Returns {} when there are no field-level
