...
// The wrapper attaches .fields to thrown errors; tests fabricate the
// same shape on a plain Error to keep fieldErrorsFrom independent of
// the ApiError class.
function withFields(fields: unknown): Error {
  const err = new Error('validation')
  ;(err as Error & { fields?: unknown }).fields = fields
  return err
}
...
describe('fieldErrorsFrom', () => {
  ...
  it('skips entries without a field key', () => {
    const err = withFields([{ rule: 'decode', message: 'bad json' }])
    expect(fieldErrorsFrom(err)).toEqual({})
  })
})
